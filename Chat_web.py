...
with st.container():
    ...
    with column2:
        ...
        if 'user' not in st.session_state:
            st.session_state['user']=[]
        with container3:
            q2="với những thông tin và giá mở phiên "+ str(Y_train[0]) +" thì mã cổ phiếu này có ưu thế gì?"
            selected_question=st.selectbox('câu hỏi:',('với những thông tin đó có những rủi ro nào?',q2))
            print(selected_question)
            #du lieu du doan
            #tao thu vien chat
        # if 'bot' not in st.session_state:
